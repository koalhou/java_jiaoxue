package com.huasoft.interceptor;
...
import com.huasoft.entity.UserInfoKey;
...
//	private EtagService etagService;
	...
	public void handleMessage(Message message) throws Fault {
		...
		if (!isExistToken || !StringUtils.hasText(token)) {
			...
			if (checkToken(token)) {
				try {
//					//TODO 先从缓存获取，没有再查询数据库
//					String key=CachedCommon.CACHED_TOKEN_KEY+token;
//					UserSeesion session=null;//(UserSeesion)etagService.get(key);
//					if(session==null){
...
//								response = Response.status(Response.Status.NOT_MODIFIED).header("Content-Type", "application/json;charset=UTF-8").build();
//							}
//						}
...
//					}
	 ...
	 * 
	 * @param reqHeaders HTTP请求Header信息.
	 * @param userID 与token匹配的终端用户信息
	 * @return 添加有终端用户信息的HTTP请求Header信息.
	 */
	private Map<String, List<String>> addUserInfo2HttpHeaderInfo(Map<String, List<String>> reqHeaders, String userID) {
